...
import { generateResultPDFWithUI } from '../../ml_api/services/pdfGeneratorPuppeteer.js';
...
router.get('/report/:sessionId', async (req, res) => {
  try {
    // Generate PDF using Puppeteer for exact UI match
    const pdfPath = await generateResultPDFWithUI(req.params.sessionId);
    res.download(pdfPath);
  } catch (err) {
    console.error('Error generating PDF with UI:', err);
